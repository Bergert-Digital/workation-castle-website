<?php
/**
 * Title: Imprint
 * Slug: workation/imprint
 * Categories: workation
 * Description: Legal imprint / company information (Italian art. 2250 c.c. data).
 * Inserter: no
 */
// phpcs:ignoreFile -- block pattern content (verbatim block markup).
?>
<!-- wp:workation/page-hero {"align":"full","eyebrow":"Legal","headline":"Imprint","lead":"Company information for the operator of Workation Castle, as required under Italian law.","imageUrl":"https://workationcastle.com/wp-content/uploads/2026/07/Castello-towards-Lugano.jpg","imageAlt":"View from Castello Carlazzo over the valley towards Lugano"} /-->

<!-- wp:group {"className":"wc-wrap","layout":{"type":"constrained"}} -->
<div class="wp-block-group wc-wrap is-layout-constrained wp-block-group-is-layout-constrained">
<!-- wp:pediment/prose -->
<!-- wp:paragraph -->
<p>The Workation Castle is operated by:</p>
<!-- /wp:paragraph -->

<!-- wp:heading -->
<h2 class="wp-block-heading">Workation Castle s.r.l.</h2>
<!-- /wp:heading -->

<!-- wp:list -->
<ul class="wp-block-list"><!-- wp:list-item -->
<li><strong>Registered office:</strong> Via Giuseppe Parini 2, 20121 Milano, Italy</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li><strong>Partita IVA / Codice Fiscale:</strong> 12713370968</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li><strong>Registro delle Imprese di Milano Monza Brianza Lodi, REA no.:</strong> MI-2680059</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li><strong>Share capital:</strong> € 10,000.00, fully paid in (interamente versato)</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li><strong>Legal representatives:</strong> Barbara Aline Buffetti and Jonas Michel Bergert</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li><strong>Email:</strong> <a href="mailto:info@workationcastle.com">info@workationcastle.com</a></li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li><strong>PEC:</strong> <a href="mailto:workationcastlesrl@pro-pec.it">workationcastlesrl@pro-pec.it</a></li>
<!-- /wp:list-item --></ul>
<!-- /wp:list -->

<!-- wp:heading -->
<h2 class="wp-block-heading">Property location</h2>
<!-- /wp:heading -->

<!-- wp:paragraph -->
<p>The Workation Castle is located at Via Castello 12, 22010 Carlazzo, Italy.</p>
<!-- /wp:paragraph -->

<!-- wp:heading -->
<h2 class="wp-block-heading">Attribution</h2>
<!-- /wp:heading -->

<!-- wp:paragraph -->
<p>Photo of Cusino (Val Cavargna): via <a href="https://de.wikipedia.org/wiki/Val_Cavargna#/media/Datei:Cusino.JPG">Wikipedia</a>.</p>
<!-- /wp:paragraph -->

<!-- wp:paragraph -->
<p><a href="/privacy-policy/">Privacy Policy →</a></p>
<!-- /wp:paragraph -->
<!-- /wp:pediment/prose -->
</div>
<!-- /wp:group -->
